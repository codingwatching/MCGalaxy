...
namespace MCGalaxy.Core {
    ...
    internal static class ChatHandler {
        ...
        static string PonyName(Player p) {
            if (p.pronouns.ThirdPersonObjectiveSingular.CaselessEq("him")) return "Brony";
            if (p.pronouns.ThirdPersonObjectiveSingular.CaselessEq("her")) return "Pegasister";
            return "Pony";
        }
        internal static void HandleCommand(Player p, string cmd, string args, CommandData data) {
            ...
            if (cmd == "pony") {
                ...
                if (used < 2) {
                    Chat.MessageFrom(p, "λNICK &Sjust so happens to be a proud " + PonyName(p).ToLower() + "! Everyone give " + p.pronouns.ThirdPersonObjectiveSingular + " &Sa brohoof!");
                    Logger.Log(LogType.CommandUsage, "{0} used /{1}", p.name, cmd);
                } else {
                    p.Message("You have used this command 2 times. You cannot use it anymore! Sorry, {0}!", PonyName(p));
                }
